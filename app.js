...
const CHAT_BTNS = [
  ...
  { btn: "全聯綁定率怎麼從 13% 拉到 42%？", q: "全聯的 LINE OA 綁定率是怎麼從 13% 提升到 42% 的？請具體說明你的做法與關鍵決策。" },
  { btn: "Silvia 的優勢是什麼？", q: "從設計轉 PM，Silvia 最大的優勢是什麼？為什麼公司該考慮錄取她？" },
];
...
function buildChips() {
  ...
  CHAT_BTNS.forEach((c) => {
    ...
    b.textContent = c.btn;
    b.addEventListener("click", () => enqueue(c.q, c.a || aiAnswer(c.q)));
    wrap.appendChild(b);
  });
}
